...
function App() {
    ...
    const handleSend = async () => {
        ...
        // Call the OpenAI API for the user's input
    ...
    const handleQuery = async (e) => {
        ...
        // Directly respond to specific queries without calling the API
        if (text.toLowerCase().includes("what is programming")) {
            setMessages([
                ...messages,
                { text: "Programming is the process of designing and building an executable computer program to accomplish a specific task.", isBot: true }
            ]);
            return; // Return to avoid calling the API for this specific query
        }

        if (text.toLowerCase().includes("how to use an api")) {
            setMessages([
                ...messages,
                { text: "To use an API (Application Programming Interface), you typically need to make HTTP requests to specific endpoints with the required parameters. The API documentation will guide you on the available endpoints, request methods, and data formats.", isBot: true }
            ]);
            return; // Return to avoid calling the API for this specific query
        }
